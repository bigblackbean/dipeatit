...
import {
  ...
  Clipboard, ClipboardCheck, Copy, Eye, EyeOff, FolderOpen, GraduationCap,
...
import { SESSION_KEY, authenticate, restoreSession, accessKey, canOpenSession, resolveViewerLocation, localDateKey } from "@/src/lectures/access";
...
export function LearningApp() {
  ...
  const courseConfig = findCourse(auth?.role === "student" ? auth.courseId : selectedCourseId) ?? courses[0];
  const hasAccess = (key: string) => sessionStorage.getItem(key) === "true";

  const applySession = useCallback((sessionInfo: SessionInfo) => {
    setAuth(sessionInfo);
    const location = resolveViewerLocation(sessionInfo, new URLSearchParams(window.location.search), key => sessionStorage.getItem(key) === "true");
    const course = location?.course ?? (sessionInfo.role === "student" ? findCourse(sessionInfo.courseId)! : courses[0]);
    ...
    setScreen(location ? "viewer" : sessionInfo.role === "admin" ? "categories" : "student");
  ...
  return (
    <>
      <DashboardShell role={role} courseConfig={courseConfig} active={screen} onNavigate={setScreen} onLogout={logout}>
        {role === "admin" && screen === "categories" && <AdminCategories onOpen={(id) => { setSelectedCategoryId(id); setScreen("courses"); }} />}
        {role === "admin" && screen === "courses" && <AdminCourses key={selectedCategoryId} categoryId={selectedCategoryId} onBack={() => setScreen("categories")} onOpen={(course) => { setSelectedCourseId(course.id); setSelectedSessionId(course.sessions[0].id); setSectionIndex(0); setScreen("course"); }} />}
        {role === "admin" && screen === "course" && <AdminCourse key={courseConfig.id} courseConfig={courseConfig} onBack={() => setScreen("courses")} onOpenSession={(session) => { setSelectedSessionId(session.id); setScreen("session"); }} />}
        {role === "admin" && screen === "session" && <AdminSession key={`${courseConfig.id}:${selectedSession.id}`} session={selectedSession} onBack={() => setScreen("course")} onOpen={openViewer} />}
        {role === "student" && <StudentDashboard courseConfig={courseConfig} onOpen={openViewer} />}
      </DashboardShell>
      {lockedSession && <AccessModal session={lockedSession} onClose={() => setLockedSession(null)} onSuccess={() => { sessionStorage.setItem(accessKey(courseConfig.id, lockedSession.id), "true"); const target = lockedSession; setLockedSession(null); openViewer(target); }} />}
    </>
  );
}

function LoginScreen({ onLogin }: { onLogin: (sessionInfo: SessionInfo) => void }) {
      ...
      <section className="relative mx-auto mt-8 grid min-h-[calc(100vh-10rem)] w-full max-w-[1440px] overflow-hidden rounded-[30px] border border-white/90 bg-white shadow-[0_28px_80px_rgba(44,74,132,.12)] lg:grid-cols-[1.14fr_.86fr]">
        <div className="relative hidden overflow-hidden bg-gradient-to-br from-[#f6f9ff] via-white to-[#edf4ff] p-14 lg:flex lg:flex-col lg:justify-between">
          <div className="relative z-10 max-w-xl animate-fade-up">
            <span className="inline-flex items-center gap-2 rounded-full border border-blue-100 bg-white px-4 py-2 text-sm font-semibold text-[#3478f6] shadow-sm"><Sparkles size={15} /> IT · 디자인 실무 클래스</span>
...
function DashboardShell({ role, courseConfig, active, onNavigate, onLogout, children }: { role: Role; courseConfig: CourseConfig; active: Screen; onNavigate: (screen: Screen) => void; onLogout: () => void; children: React.ReactNode }) {
  return (
    <div className="min-h-screen bg-[#fbfcff] lg:grid lg:h-dvh lg:min-h-0 lg:grid-cols-[274px_minmax(0,1fr)] lg:grid-rows-[minmax(0,1fr)] lg:overflow-hidden">
      <aside className="soft-scrollbar hidden min-h-0 overflow-y-auto overscroll-contain border-r border-[#e7ecf3] bg-white px-5 py-7 lg:flex lg:flex-col">
        <div className="shrink-0"><BrandMark /></div>
        ...
        <div className="mt-auto shrink-0 rounded-[18px] border border-[#e1e7f0] bg-gradient-to-br from-white to-[#f5f8ff] p-4">
          <div className="flex items-center gap-2 text-sm font-bold"><GraduationCap size={18} className="text-[#3478f6]"/>{role === "admin" ? "관리자 모드" : courseConfig.title}</div>
          <p className="mt-2 text-xs leading-5 text-[#667085]">{role === "admin" ? "전체 강의와 공개 설정을 확인합니다." : `${courseConfig.totalHours ? courseConfig.totalHours + "시간 · " : ""}${courseConfig.sessions.length}회차 과정`}</p>
        </div>
        <button onClick={onLogout} className="focus-ring mt-4 flex h-11 shrink-0 items-center gap-3 rounded-xl px-3 text-sm font-semibold text-[#667085] hover:bg-[#f5f7fb] hover:text-[#172033]"><LogOut size={18}/> 로그아웃</button>
      </aside>
...
function AdminCategories({ onOpen }: { onOpen: (categoryId: string) => void }) {
  ...
  return <div className="animate-fade-up"><PageHeading eyebrow="ADMIN" title="강의 카테고리" description="운영 중인 강의를 분야별로 확인하세요."/><div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">{categories.map(category => <button key={category.id} onClick={() => category.count > 0 && onOpen(category.id)} className={`focus-ring group rounded-[22px] border border-[#e1e7f0] bg-white p-5 text-left shadow-[0_10px_30px_rgba(47,76,137,.06)] transition hover:-translate-y-1 hover:border-blue-200 hover:shadow-[0_16px_36px_rgba(47,76,137,.1)] ${category.count === 0 ? "opacity-60" : ""}`}><span className={`grid h-14 w-14 place-items-center rounded-[18px] bg-gradient-to-br shadow-lg ${toneClasses[category.tone]}`}><FolderOpen size={27} className="text-white"/></span><div className="mt-7 flex items-end justify-between"><div><h2 className="font-bold">{category.name}</h2><p className="mt-1 text-sm text-[#98a2b3]">{category.count}개 강의</p></div><ChevronRight size={20} className="text-[#bdc5d1] transition group-hover:translate-x-1 group-hover:text-[#3478f6]"/></div></button>)}</div></div>;
}

function AdminCourses({ categoryId, onBack, onOpen }: { categoryId: string; onBack: () => void; onOpen: (course: CourseConfig) => void }) {
  const [search, setSearch] = useState("");
  const [filter, setFilter] = useState("전체");
  const category = categories.find(item => item.id === categoryId);
  const visibleCourses = courses
    .filter(course => course.categoryId === categoryId && course.title.toLowerCase().includes(search.trim().toLowerCase()) && (filter === "전체" || filter === courseState(course)))
    .sort((a, b) => b.startDate.localeCompare(a.startDate));
  return (
    <div className="animate-fade-up">
      <PageHeading eyebrow={category?.name} title="강의 목록" description="강의명으로 검색하거나 진행 상태를 선택할 수 있습니다." back={onBack}/>
      <div className="mb-5 flex flex-col gap-3 rounded-[18px] border border-[#e3e8f0] bg-white p-4 sm:flex-row">
        <label className="flex flex-1 items-center gap-3 rounded-xl bg-[#f7f9fc] px-4 text-[#98a2b3]">
          <Search size={18}/><input value={search} onChange={e=>setSearch(e.target.value)} className="h-11 w-full bg-transparent text-sm text-[#172033] outline-none" placeholder="강의명 검색"/>
        </label>
        <div className="flex items-center gap-2 overflow-x-auto">
          <ListFilter size={17} className="mr-1 shrink-0 text-[#98a2b3]"/>
          {["전체","예정","진행중","종료"].map(item=><button key={item} onClick={()=>setFilter(item)} className={`focus-ring shrink-0 rounded-lg px-3.5 py-2 text-xs font-bold ${filter===item?"bg-[#3478f6] text-white":"bg-[#f7f9fc] text-[#667085] hover:bg-[#edf3ff]"}`}>{item}</button>)}
        </div>
      </div>
      <div className="space-y-3">
        {visibleCourses.length ? visibleCourses.map(course => (
          <button key={course.id} onClick={()=>onOpen(course)} className="focus-ring group grid w-full gap-4 rounded-[20px] border border-[#e1e7f0] bg-white p-5 text-left transition hover:border-blue-200 hover:shadow-[0_12px_34px_rgba(47,76,137,.08)] sm:grid-cols-[1fr_180px_100px_32px] sm:items-center">
            <div><p className="font-bold tracking-[-.02em]">{course.title}</p><p className="mt-2 text-sm text-[#667085]">{formatDate(course.startDate)} ~ {formatDate(course.endDate)}</p></div>
            <span className="text-sm text-[#667085]">총 {course.sessions.length}회{course.totalHours ? ` · ${course.totalHours}시간` : ""}</span>
            <StatusBadge status={courseState(course)}/><ChevronRight className="text-[#b6c0ce] group-hover:text-[#3478f6]"/>
          </button>
        )) : <div className="rounded-[20px] border border-dashed border-[#dce3ed] bg-white py-20 text-center text-sm text-[#98a2b3]">검색 결과가 없습니다.</div>}
      </div>
    </div>
  );
}
...
function AdminCourse({ courseConfig, onBack, onOpenSession }: { courseConfig: CourseConfig; onBack: () => void; onOpenSession: (session: SessionConfig) => void }) {
  const [showPassword, setShowPassword] = useState(false);
  ...
  return <div className="animate-fade-up"><PageHeading eyebrow={courseConfig.categoryName} title={courseConfig.title} description={`${formatDate(courseConfig.startDate)} ~ ${formatDate(courseConfig.endDate)}`} back={onBack}/><div className="mb-6 grid gap-4 sm:grid-cols-3"><StatCard label="진행 상태" value={courseState(courseConfig)} icon={<CalendarDays/>}/><StatCard label="현재 진행 회차" value={`${released} / ${courseConfig.sessions.length}`} icon={<BookOpen/>}/><StatCard label={courseConfig.totalHours ? "전체 수업시간" : "전체 회차"} value={courseConfig.totalHours ? `${courseConfig.totalHours}시간` : `${courseConfig.sessions.length}회`} icon={<GraduationCap/>}/></div><section className="mb-7 rounded-[20px] border border-[#dfe6f0] bg-white p-5 sm:p-6"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-center"><div><p className="text-xs font-bold text-[#98a2b3]">수강생 공통계정</p><div className="mt-2 flex flex-wrap items-center gap-3"><code className="rounded-lg bg-[#f4f7fc] px-3 py-2 text-sm font-bold text-[#3478f6]">ID {courseConfig.studentAccount.id}</code><code className="rounded-lg bg-[#f4f7fc] px-3 py-2 text-sm font-bold">PW {showPassword ? courseConfig.studentAccount.password : "••••••••••"}</code></div></div><div className="flex gap-2"><button onClick={()=>setShowPassword(!showPassword)} className="focus-ring grid h-10 w-10 place-items-center rounded-xl border border-[#dfe6f0] text-[#667085] hover:text-[#3478f6]" aria-label="비밀번호 보기">{showPassword?<EyeOff size={18}/>:<Eye size={18}/>}</button><button onClick={copyPassword} className="focus-ring flex h-10 items-center gap-2 rounded-xl border border-[#dfe6f0] px-3 text-xs font-bold text-[#667085] hover:text-[#3478f6]">{copied?<ClipboardCheck size={17}/>:<Copy size={17}/>} {copied?"복사됨":"복사"}</button></div></div></section><SessionList courseConfig={courseConfig} admin onOpen={onOpenSession}/></div>;
}
...
function SessionList({ courseConfig, admin = false, onOpen }: { courseConfig: CourseConfig; admin?: boolean; onOpen: (session: SessionConfig) => void }) {
...
function LectureViewer({ role, courseConfig, session, initialIndex, onIndexChange, onExit }: { role: Role; courseConfig: CourseConfig; session: SessionConfig; initialIndex: number; onIndexChange: (index:number)=>void; onExit:()=>void }) {
  ...
  const go=useCallback((next:number)=>{const safe=Math.max(0,Math.min(slides.length-1,next));setIndex(safe);onIndexChange(safe);setMenuOpen(false);const params=new URLSearchParams();params.set("course",courseConfig.id);params.set("session",session.id);params.set("section",String(safe+1));window.history.pushState(null,"",`?${params.toString()}`);},[onIndexChange,courseConfig.id,session.id,slides.length]);

  useEffect(()=>{const params=new URLSearchParams();params.set("course",courseConfig.id);params.set("session",session.id);params.set("section",String(index+1));window.history.replaceState(null,"",`?${params.toString()}`);},[index,courseConfig.id,session.id]);
